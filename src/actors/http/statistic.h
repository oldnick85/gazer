...
public:
    ...
    uint Min_usec() const
    {
        return std::chrono::duration_cast<std::chrono::microseconds>(m_min).count();
    }

    uint Max_usec() const
    {
        return std::chrono::duration_cast<std::chrono::microseconds>(m_max).count();
    }

    uint Mean_usec() const
    {
        return std::chrono::duration_cast<std::chrono::microseconds>(m_mean).count();
    }

    uint Sigm_usec() const
    {
        return std::chrono::duration_cast<std::chrono::microseconds>(m_sigm).count();
    }

    std::string to_str() const
    {
        std::string s = std::string("min=") + std::to_string(std::chrono::duration_cast<std::chrono::microseconds>(m_min).count()) + " " 
...
public:
    ...
    ~CDurationWatch() 
    {
        m_d->AddTime(Time_t::now() - m_start);
    }
private:
...
public:
    void NewRegistry() 
    {
        m_requests_registry.Add();
    }

    void NewVerify() 
    {
        m_requests_verify.Add();
    }

    void RegistryDuration(const Duration_t d)
    {
        m_duration_registry.AddTime(d);
    }

    void VerifyDuration(const Duration_t d)
    {
        m_duration_verify.AddTime(d);
    }
    ...
    const CCounter& RegistryCounter() const
    {
        return m_requests_registry;
    }

    const CCounter& VerifyCounter() const
    {
        return m_requests_verify;
    }

    const CDuration& RegistryDuration() const
    {
        return m_duration_registry;
    }

    const CDuration& VerifyDuration() const
    {
        return m_duration_verify;
    }
