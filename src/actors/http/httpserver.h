...
private:
    void BindHandlers();
    std::string PlotPseudoRaw(const Value& value);
    std::string PlotPseudo(const std::vector<float>& values, std::vector<std::chrono::time_point<std::chrono::system_clock>> times, const std::string caption);
